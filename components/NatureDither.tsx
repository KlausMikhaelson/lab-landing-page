...
function startScene(
  ...
  console.info(`[General Simulation] scene renderer r5: webgl1, live-recreate, dpr cap ${dprCap}`);
  ...
  const onLost = (e: Event) => {
    ...
    if (lossCount >= 3) {
      ...
      console.warn("[General Simulation] WebGL context thrashing, recreating at lower resolution");
      die();
      return;
    }
    ...
    restoreTimer = setTimeout(() => {
      if (!program) {
        console.warn("[General Simulation] WebGL context not restored, recreating canvas");
        die();
      }
    }, 1500);
  };
